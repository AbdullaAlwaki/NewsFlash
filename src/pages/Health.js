...
function Health() {
    const {health,setShowMore,showMore,btnRef,numb, spin,setNumb} = useContext(Data);
  
  return (
    <div className="App">
      {spin ? <p className="load">Loading...</p>: ""}
      {health.slice(0, numb).map((n, i) => {
        return (
          <div key={i} >
            <h3>{i + 1}-{n.title}</h3>
            <p>{n.author}</p>
            ...
            {n.urlToImage == null ? <p>sorry no img hire because we have free Api 🥲</p>: <img src={n.urlToImage} alt="" width={"250px"} />}
            <button
              onClick={() => {
